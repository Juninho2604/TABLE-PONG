...
export async function getFinancialSummaryAction(month?: number, year?: number): Promise<{
  ...
  try {
    ...
    const expenses = await prisma.expense.findMany({
      where: { deletedAt: null, status: 'CONFIRMED', periodMonth: m, periodYear: y },
    ...
    // 4. Cuentas por pagar pendientes (todo historial)
...
export async function getMonthlyTrendAction(months = 6): Promise<{
  ...
  try {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const startDate = new Date(yr, m - 1, 1);
      const endDate = new Date(yr, m, 0, 23, 59, 59, 999);
      ...
      const [salesAgg, expAgg] = await Promise.all([
        ...
        prisma.expense.aggregate({
          where: { deletedAt: null, status: 'CONFIRMED', periodMonth: m, periodYear: yr },
          _sum: { amountUsd: true },
        }),
      ]);
      ...
      results.push({
        label: `${MONTH_NAMES[m - 1].slice(0, 3)} ${yr}`,
        sales,
        expenses,
