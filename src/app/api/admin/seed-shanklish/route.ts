...
// Endpoint de un solo uso para cargar el menú Shanklish Caracas
// Llama a: GET /api/admin/seed-shanklish?key=shanklish2024

export async function GET(request: Request) {
    const { searchParams } = new URL(request.url);
    if (searchParams.get('key') !== 'shanklish2024') {
    ...
    try {
        ...
        return NextResponse.json({
            success: true,
            message: `✅ Menú Shanklish Caracas cargado. Creados: ${created} | Ya existían: ${skipped}`,
            categoryId,
            created,
            skipped,
        });
